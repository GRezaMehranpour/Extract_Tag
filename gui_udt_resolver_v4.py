...
# In gui_udt_resolver_v3.py
    ...
    def _resolve_db_tags(self, db_node, udt_dict):
        ...
        # --- SCENARIO 1: The DB is an instance of a UDT ---
        # e.g., DATA_BLOCK "MyInstanceDB" "MyUDT"
        implicit_type_node = db_node.child_by_field_name('type')
        if implicit_type_node:
            # The base_path is just the DB name, and we expand its type.
            self._expand_tag(db_name, implicit_type_node, resolved_tags, udt_dict)
            return resolved_tags

        # --- SCENARIO 2: The DB contains an inline STRUCT definition ---
        # This is common in DBs generated from older systems or some code styles.
        # e.g., DATA_BLOCK "MyDb" STRUCT ... END_STRUCT
            ...
            for field_node in struct_def_node.children:
                if field_node.type == 'fields':
                    ...
                    if tag_name and tag_type_node:
                        base_path = f"{db_name}.{tag_name}"
                        self._expand_tag(base_path, tag_type_node, resolved_tags, udt_dict)
            return resolved_tags

        # --- SCENARIO 3: The DB is a global DB with VAR sections (YOUR CASE) ---
        # e.g., DATA_BLOCK "MyGlobalDB" VAR ... END_VAR
        ...
        # If none of the above, it's an empty DB.
 ...
 # In gui_udt_resolver_v3.py
    ...
    def _expand_tag(self, base_path, type_node, resolved_tags, udt_dict):
        ...
        # --- Array Handling (MODIFIED SECTION) ---
        ...
        if array_def_node:
            ...
            # Find all potential dimension bounds: integers or identifiers (constants)
            dim_nodes = [c for c in array_def_node.children if c.type in ('integer_literal', 'identifier')]
            
            if len(dim_nodes) > 0 and len(dim_nodes) % 2 == 0:
                dimensions_are_numeric = True
                dimensions = []
                
                try:
                    for i in range(0, len(dim_nodes), 2):
                        start_text = get_node_text(dim_nodes[i])
                        end_text = get_node_text(dim_nodes[i+1])
                        
                        # Try to convert to int, but keep as string if it's a constant
                        try:
                            start = int(start_text)
                            end = int(end_text)
                            dimensions.append((start, end))
                        except ValueError:
                            # This happens if a bound is an identifier like "cMAX_ALARMS"
                            dimensions_are_numeric = False
                            # Store the text representation instead
                            dimensions.append((start_text, end_text))

                    # Only expand if all dimensions were successfully parsed as numbers
                    if dimensions_are_numeric:
                        self._expand_array_dimensions(base_path, dimensions, base_type_node_of_array, resolved_tags, udt_dict)
                        return
                    else:
                        # If any dimension is a constant, just show the placeholder ranges
                        dim_str = ",".join([f"{s}..{e}" for s, e in dimensions])
                        self._expand_tag(f"{base_path}[{dim_str}]", base_type_node_of_array, resolved_tags, udt_dict)
                        return

                except IndexError as e:
                    self.queue.put(("log_error", f"Could not parse array dimensions for '{base_path}': {e}"))
            
            # Fallback for parsing failure or complex cases
            self._expand_tag(f"{base_path}[]", base_type_node_of_array, resolved_tags, udt_dict)
            return

        # --- Base Type / UDT Handling (Unchanged) ---
